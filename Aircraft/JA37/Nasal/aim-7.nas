...
var aim_7_model    = "Aircraft/JA37/Models/Armament/Weapons/RB-71/rb-71-";
var SwSoundOnOff   = AcModel.getNode("armament/rb71/sound-on-off");
var SwSoundVol     = AcModel.getNode("armament/rb71/sound-volume");
var vol_search     = 0.00;
...
var AIM7 = {
	...
	new : func (p) {
		...
		m.prop              = AcModel.getNode("armament/rb71/").getChild("msl", 0 , 1);
		...
		# AIM-7L specs:
		m.aim7_fov_diam     = getprop("sim/ja37/armament/rb71/fov-deg");
		m.aim7_fov          = m.aim7_fov_diam / 2;
		m.max_detect_rng    = getprop("sim/ja37/armament/rb71/max-detection-rng-nm");
		m.max_seeker_dev    = getprop("sim/ja37/armament/rb71/track-max-deg") / 2;
		m.force_lbs         = getprop("sim/ja37/armament/rb71/thrust-lbs");
		m.thrust_duration   = getprop("sim/ja37/armament/rb71/thrust-duration-sec");
		m.weight_launch_lbs = getprop("sim/ja37/armament/rb71/weight-launch-lbs");
		m.weight_whead_lbs  = getprop("sim/ja37/armament/rb71/weight-warhead-lbs");
		...
		for (i = 0; 1==1; i += 1) {
			if (n.getChild("rb-71", i, 0) == nil) {
				break;
			}
		}
		m.ai = n.getChild("rb-71", i, 1);

		m.ai.getNode("valid", 1).setBoolValue(1);
		m.ai.getNode("name", 1).setValue("RB-71");
		m.ai.getNode("sign", 1).setValue("Skyflash");
		#m.model.getNode("collision", 1).setBoolValue(0);
		#m.model.getNode("impact", 1).setBoolValue(0);
		var id_model = aim_7_model ~ m.ID ~ ".xml";
	...
	del: func {
		...
		me.ai.remove();
		if (me.status == 2) {
			delete(AIM7.flying, me.flyID);
		} else {
			delete(AIM7.active, me.ID);
		}
	},
	#done
	release: func() {
	...
	update: func {
		...
		if (me.life_time > 120) { return me.del(); }
...
#print ("**** curr_tgt_e = ", me.curr_tgt_e," curr_tgt_h = ", me.curr_tgt_h, " me.track_signal_e = ", me.track_signal_e," me.track_signal_h = ", me.track_signal_h);
	...
	check_t_in_fov: func {
		...
		var e_d = me.seeker_dev_e - me.aim7_fov;
		var e_u = me.seeker_dev_e + me.aim7_fov;
		var h_l = me.seeker_dev_h - me.aim7_fov;
		var h_r = me.seeker_dev_h + me.aim7_fov;
	...
	animation_flags_props: func {
		...
		var msl_path = "sim/ja37/armament/rb71/flags/msl-id-" ~ me.ID;
		me.msl_prop = props.globals.initNode( msl_path, 1, "BOOL" );
		var smoke_path = "sim/ja37/armament/rb71/flags/smoke-id-" ~ me.ID;
		me.smoke_prop = props.globals.initNode( smoke_path, 0, "BOOL" );
		var explode_path = "sim/ja37/armament/rb71/flags/explode-id-" ~ me.ID;
		me.explode_prop = props.globals.initNode( explode_path, 0, "BOOL" );
		var explode_smoke_path = "sim/ja37/armament/rb71/flags/explode-smoke-id-" ~ me.ID;
		me.explode_smoke_prop = props.globals.initNode( explode_smoke_path, 0, "BOOL" );
		var explode_sound_path = "sim/ja37/armament/rb71/flags/explode-sound-on-" ~ me.ID;;
		me.explode_sound_prop = props.globals.initNode( explode_sound_path, 0, "BOOL" );
		var explode_sound_vol_path = "sim/ja37/armament/rb71/flags/explode-sound-vol-" ~ me.ID;;
