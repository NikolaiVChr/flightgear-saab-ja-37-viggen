...
var tacview_ac_type = variant.JA ? "JA-37" : "AJS-37";
# Aircraft type as inserted in the output file name
var filename_ac_type = variant.JA ? "ja37" : "ajs37";
...
var get_contacts_list = func {
    return radar.get_complete_list();
}

# Function returning the focused/locked aircraft, as a "contact" object (or nil).
var get_primary_contact = variant.JA ? func { return radar.ps46.getPriorityTarget(); } : func { return nil; };

# Radar range. May return nil if n/a
var get_radar_range_nm = variant.JA ? func { return radar.ps46.getRange(); } : func { return radar.ps37.getRange(); };
...
var getTas = func() {
    ...
    if (tas != nil)
        return sprintf("%.0f", tas * KT2MPS);
    else
        return nil;
}

var getCas = func() {
    return sprintf("%.0f", input.cas.getValue() * KT2MPS);
}
