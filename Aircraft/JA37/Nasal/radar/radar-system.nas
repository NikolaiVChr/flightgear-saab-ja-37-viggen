...
#   replaceNode() [in AI tree]
	...
	# Counterintuitively the radar is disabled, when the MP property sim/multiplay/generic/int[2]
	# is set to 1. Otherwise it is enabled.
	isRadarEnable: func {
...
#
	...
	# Get the coordinate with updated elevation info used to calculate visibility
	# Most assets sit on the ground at z=0. Then even small terrain bumbs hide them
	# - despite the fact that they have a certain height and therefore visibility actually should
	# be based on their shoulder height over ground.
	get_coord_for_view: func(anti_rad) {
		...
		if (anti_rad == TRUE and me.isRadarEnable() and isOmniRadiating(me.model)) {
			...
			extra_height = 1.5;
		} elsif (me.get_type() == MARINE) {
			# we add height even though the ocean is flat - but the earth is round
			# and in WS20 not all parts of the ocean have elevation=0 - som are -1 or even less.
			extra_height = 2.5;
		}
		updated_coord.set_xyz(updated_coord.x(), updated_coord.y(), updated_coord.z() + extra_height);
